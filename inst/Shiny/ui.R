      ...
      tabItem(
        ...
        fluidPage(
          ...
          div(
            ...
            lapply(
              list(
                list(title = "Canvas", desc = "Define the model’s environment using a drag-and-drop interface.", img = "Canvas.png", tab = "canvas_tab"),
                list(title = "Rooms", desc = "Define new room types.", img = "Rooms.png", tab = "rooms"),
                list(title = "Agents", desc = "Define agent types and their movement logic.", img = "Agents.png", tab = "agents"),
                list(title = "Resources", desc = "Specify room capacity per agent type.", img = "Resources.png", tab = "resources"),
                list(title = "Infection", desc = "Define the disease model used in simulation.", img = "Infection.png", tab = "infection"),
                list(title = "What-If", desc = "Perform what-if analysis with countermeasures and virus settings.", img = c("Countermeasures.png", "Virus.png"), tab = "whatif"),
                list(title = "Configuration", desc = "Set up the initial configuration for simulation.", img = "Configuration.png", tab = "configuration"),
                list(title = "Run", desc = "Execute the simulation with defined parameters.", img = "Run.png", tab = "run"),
                list(title = "Settings", desc = "Adjust canvas size, load, and save models.", img = "Settings.png", tab = "settings"),
                list(title = "Post Processing", desc = "Analyze simulation outputs with 2D visualization.", img = "2DVisualisation.png", tab = "post_process")
              ),
              function(card) {
                div(
                  ...
                  div(
                    ...
                    style = "
                      ...
                      flex:1;",
                    div(
                      h4(card$title, style = "margin:0 0 5px 0; padding:0;")
                    ),
                    div(
                      p(card$desc, style = "margin:0 0 10px 0; padding:0; font-size:20px; line-height:1.4; flex-shrink:0;")
                    ),
                    div(
                      style = "display:flex; flex-direction:column; gap:5px; flex-grow:1;",
                      lapply(card$img, function(i) {
                        actionLink(
                          inputId = paste0("link_", card$tab),
                          label = img(
                            src = i,
                            style = "width:100%; height:auto; border-radius:10px; display:block; margin:0; padding:0;"
                          )
                        )
                      })
                    )
