...
server <- function(input, output,session) {
  ...
  output$RoomColors <- renderUI({
    if(!is.null(canvasObjects$rooms) && input$selectInput_color_room!= ""){
      col_output_list <-  lapply(input$selectInput_color_room,function(name)
      {
        ...
        colourInput(paste0("col_",room$Name),
  ...
  output$AreaColors <- renderUI({
    if(!is.null(canvasObjects$areas) && input$selectInput_color_area!= ""){
      ...
      div(
        colourInput(paste0("col_area_",name),
  ...
  output$TypeColors <- renderUI({
    if(!is.null(canvasObjects$types) && input$selectInput_color_type!= ""){
      ...
      div(
        colourInput(paste0("col_type_",name),
                    paste0("Select colour for " , name),
                    gsub(pattern = ", 1\\)",replacement = "\\)",
                         gsub(pattern = "rgba",replacement = "rgb",color)
                    ),
  ...
  observeEvent(input$check, {
    ...
    if(!dir.exists("../../FLAMEGPU-FORGE4FLAME/resources/f4f/")){
      output$flame_link <- renderText({
        paste0("The directory ", gsub("ShinyEnvironment", "", getwd()), "resources/f4f/ does not exitst. Check you are in the correct directory.")
      })
      return()
    }
  ...
  observeEvent(input$save_text, {
    ...
    file_name <- glue("WHOLEmodel.RDs")
    saveRDS(reactiveValuesToList(canvasObjects), file=file.path(paste0("../../FLAMEGPU-FORGE4FLAME/resources/f4f/", input$popup_text), file_name))
    
    file_name <- glue("WHOLEmodel.json")
    write_json(x = reactiveValuesToList(canvasObjects), path = file.path(paste0("../../FLAMEGPU-FORGE4FLAME/resources/f4f/", input$popup_text), file_name))
    
    if(canvasObjects$whatif$outside_contagion_file != "") {
      write_csv(x = canvasObjects$outside_contagion, file = file.path(paste0("../../FLAMEGPU-FORGE4FLAME/resources/f4f/", input$popup_text), canvasObjects$whatif$outside_contagion_file))
    }
