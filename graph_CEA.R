...
read_runsAPS_from_seeds <- function(seeds) {
  ...
  run_list <- lapply(seeds, function(seed) {
    ...
    list(agentPS = df_agent, seed = seed)
  })

  return(run_list)
}

all_runs_nomask <- read_runsAPS_from_seeds(seeds_no_counter)
all_runs_surgical40 <- read_runsAPS_from_seeds(seeds_surgical40)
all_runs_surgical90 <- read_runsAPS_from_seeds(seeds_surgical90)
all_runs_ffp240 <- read_runsAPS_from_seeds(seeds_ffp240)
all_runs_ffp290 <- read_runsAPS_from_seeds(seeds_ffp290)
all_runs_quarantineswab <- read_runsAPS_from_seeds(seeds_quarantineswab)
all_runs_quarantineswab[[42]]$agentPS[327013, "disease_state"] <- "3"
all_runs_quarantineswabsur40 <- read_runsAPS_from_seeds(seeds_quarantineswabsur40)
all_runs_quarantineswabsur90 <- read_runsAPS_from_seeds(seeds_quarantineswabsur90)
all_runs_quarantineswabffp240 <- read_runsAPS_from_seeds(seeds_quarantineswabffp240)
all_runs_quarantineswabffp290 <- read_runsAPS_from_seeds(seeds_quarantineswabffp290)
...
processedAPS_from_runs <- function(all_runs, agents, disease_name, fixed_colors) {
  ...
  process_single <- function(simulation_log) {
    ...
    ref_day <- 8
    pre_days <- 1:(ref_day-1)

    # righe del giorno di riferimento
    day8_rows <- simulation_log %>%
      filter(day == ref_day, agent_type != "Patient") %>%
      select(agent_type, disease_state, count)

    # costruisci giorni 1:7 copiando day8
    day1_7_rows <- expand.grid(day = pre_days,
                               agent_type = unique(day8_rows$agent_type)) %>%
      left_join(day8_rows, by = "agent_type", relationship = "many-to-many") %>%
      group_by(agent_type, day) %>%
      mutate(count = count) %>%
      ungroup() %>%
      select(agent_type, disease_state, day, count)

    # sostituisci i giorni originali 1:7 non-Patient
    daily_complete_adjusted <- simulation_log %>%
      filter(!(agent_type != "Patient" & day %in% pre_days)) %>%
      bind_rows(day1_7_rows) %>%
      arrange(agent_type, disease_state, day)

    return(daily_complete_adjusted)
  }
...
# pl = ggplot(agentPS_nocont[[1]], aes(x = day, y = count, color = factor(disease_state), group = disease_state)) +
#   geom_line()+
...
agg_mean_area_nocont <- agentPS_nocont %>%
  bind_rows() %>%
...
agg_mean_area_surgical40 <- agentPS_surgical40 %>%
  bind_rows() %>%
  filter(!is.na(agent_type)) %>%
  group_by(agent_type, disease_state, day) %>%
  summarise(
...
agg_mean_area_surgical90 <- agentPS_surgical90 %>%
  bind_rows() %>%
  filter(!is.na(agent_type)) %>%
  group_by(agent_type, disease_state, day) %>%
  summarise(
...
agg_mean_area_ffp240 <- agentPS_ffp240 %>%
  bind_rows() %>%
  filter(!is.na(agent_type)) %>%
  group_by(agent_type, disease_state, day) %>%
  summarise(
...
agg_mean_area_ffp290 <- agentPS_ffp290 %>%
  bind_rows() %>%
  filter(!is.na(agent_type)) %>%
  group_by(agent_type, disease_state, day) %>%
  summarise(
...
agg_mean_area_quarantineswab <- agentPS_quarantineswab %>%
  bind_rows() %>%
  filter(!is.na(agent_type)) %>%
  group_by(agent_type, disease_state, day) %>%
  summarise(
...
agg_mean_area_quarantineswabsur40 <- agentPS_quarantineswabsur40 %>%
  bind_rows() %>%
  filter(!is.na(agent_type)) %>%
  group_by(agent_type, disease_state, day) %>%
  summarise(
...
agg_mean_area_quarantineswabsur90 <- agentPS_quarantineswabsur90 %>%
  bind_rows() %>%
  filter(!is.na(agent_type)) %>%
  group_by(agent_type, disease_state, day) %>%
  summarise(
...
agg_mean_area_quarantineswabffp240 <- agentPS_quarantineswabffp240 %>%
  bind_rows() %>%
  filter(!is.na(agent_type)) %>%
  group_by(agent_type, disease_state, day) %>%
  summarise(
...
agg_mean_area_quarantineswabffp290 <- agentPS_quarantineswabffp290 %>%
  bind_rows() %>%
  filter(!is.na(agent_type)) %>%
  group_by(agent_type, disease_state, day) %>%
  summarise(
...
#   labs(x = "Giorno", y = "Numero agenti", title = "No Countermeasures") +
#   scale_color_manual(values = fixed_colors)
# theme(legend.position = "bottom")
# p_area


p_mean_nocont <- ggplot() +
  geom_line(data = agg_mean_area_nocont, aes(x = day, y = MeanV, color = disease_state), size = 1) +
  geom_ribbon(data = agg_mean_area_nocont, aes(x = day, ymin = MinV, ymax = MaxV, fill = disease_state), alpha = 0.25, colour = NA) +
  facet_wrap(~ agent_type, scales = "free_y") +
  scale_color_manual(values = fixed_colors, name = "Mean") +
  labs(x = "Days", y = "Agent Number", title = "No Countermeasures") +
  theme_minimal() +
  theme(legend.position = "bottom")+
  scale_x_continuous(limits = c(1, 100))
p_mean_nocont
...
p_mean_sur40 <- ggplot() +
  geom_line(data = agg_mean_area_surgical40, aes(x = day, y = MeanV, color = disease_state), size = 1) +
  geom_ribbon(data = agg_mean_area_nocont, aes(x = day, ymin = MinV, ymax = MaxV, fill = disease_state), alpha = 0.25, colour = NA) +
  facet_wrap(~ agent_type, scales = "free_y") +
  scale_color_manual(values = fixed_colors, name = "Mean") +
  labs(x = "Days", y = "Agent Number", title = "Surgical Mask 40%") +
  theme_minimal() +
  theme(legend.position = "bottom")+
  scale_x_continuous(limits = c(1, 100))
p_mean_sur40
...
p_mean_sur90 <- ggplot() +
  geom_line(data = agg_mean_area_surgical90, aes(x = day, y = MeanV, color = disease_state), size = 1, linetype = "dashed") +
  facet_wrap(~ agent_type, scales = "free_y") +
  scale_color_manual(values = fixed_colors, name = "Mean") +
  labs(x = "Days", y = "Agent Number", title = "Surgical Mask 90%") +
  theme_minimal() +
  theme(legend.position = "bottom")+
  scale_x_continuous(limits = c(1, 100))
p_mean_sur90

p_mean_ffp240 <- ggplot() +
  geom_line(data = agg_mean_area_ffp240, aes(x = day, y = MeanV, color = disease_state), size = 1, linetype = "dashed") +
  facet_wrap(~ agent_type, scales = "free_y") +
  scale_color_manual(values = fixed_colors, name = "Mean") +
  labs(x = "Days", y = "Agent Number", title = "FFP2 Mask 40%") +
  theme_minimal() +
  theme(legend.position = "bottom")+
  scale_x_continuous(limits = c(1, 100))
p_mean_ffp240

p_mean_ffp290 <- ggplot() +
  geom_line(data = agg_mean_area_ffp290, aes(x = day, y = MeanV, color = disease_state), size = 1, linetype = "dashed") +
  facet_wrap(~ agent_type, scales = "free_y") +
  scale_color_manual(values = fixed_colors, name = "Mean") +
  labs(x = "Days", y = "Agent Number", title = "FFP2 Mask 90%") +
  theme_minimal() +
  theme(legend.position = "bottom")+
  scale_x_continuous(limits = c(1, 100))
p_mean_ffp290

p_mean_quarantine <- ggplot() +
  geom_line(data = agg_mean_area_quarantineswab, aes(x = day, y = MeanV, color = disease_state), size = 1, linetype = "dashed") +
  facet_wrap(~ agent_type, scales = "free_y") +
  scale_color_manual(values = fixed_colors, name = "Mean") +
  labs(x = "Days", y = "Agent Number", title = "Quarantine, swabs, no masks") +
  theme_minimal() +
  theme(legend.position = "bottom")+
  scale_x_continuous(limits = c(1, 100))
p_mean_quarantine

p_mean_quarantinesur40 <- ggplot() +
  geom_line(data = agg_mean_area_quarantineswabsur40, aes(x = day, y = MeanV, color = disease_state), size = 1, linetype = "dashed") +
  facet_wrap(~ agent_type, scales = "free_y") +
  scale_color_manual(values = fixed_colors, name = "Mean") +
  labs(x = "Days", y = "Agent Number", title = "Quarantine, swabs, Surgical mask 40%") +
  theme_minimal() +
  theme(legend.position = "bottom")+
  scale_x_continuous(limits = c(1, 100))
p_mean_quarantinesur40

p_mean_quarantinesur90 <- ggplot() +
  geom_line(data = agg_mean_area_quarantineswabsur90, aes(x = day, y = MeanV, color = disease_state), size = 1, linetype = "dashed") +
  facet_wrap(~ agent_type, scales = "free_y") +
  scale_color_manual(values = fixed_colors, name = "Mean") +
  labs(x = "Days", y = "Agent Number", title = "Quarantine, swabs, Surgical mask 90%") +
  theme_minimal() +
  theme(legend.position = "bottom")+
  scale_x_continuous(limits = c(1, 100))
p_mean_quarantinesur90

p_mean_quarantineffp240 <- ggplot() +
  geom_line(data = agg_mean_area_quarantineswabffp240, aes(x = day, y = MeanV, color = disease_state), size = 1, linetype = "dashed") +
  facet_wrap(~ agent_type, scales = "free_y") +
  scale_color_manual(values = fixed_colors, name = "Mean") +
  labs(x = "Days", y = "Agent Number", title = "Quarantine, swabs, FFP2 mask 40%") +
  theme_minimal() +
  theme(legend.position = "bottom")+
  scale_x_continuous(limits = c(1, 100))
p_mean_quarantineffp240

p_mean_quarantineffp290 <- ggplot() +
  geom_line(data = agg_mean_area_quarantineswabffp290, aes(x = day, y = MeanV, color = disease_state), size = 1, linetype = "dashed") +
  facet_wrap(~ agent_type, scales = "free_y") +
  scale_color_manual(values = fixed_colors, name = "Mean") +
  labs(x = "Days", y = "Agent Number", title = "Quarantine, swabs, FFP2 mask 90%") +
  theme_minimal() +
  theme(legend.position = "bottom")+
  scale_x_continuous(limits = c(1, 100))
p_mean_quarantineffp290
...
ggplot(icer_runs_plot, aes(x = delta_qaly, y = delta_cost, color = ic_status)) +
  ...
  #
  # Punto nero → mean ΔQALY e ΔCost su tutte le run → rappresenta l’andamento “tipico”
  #
...
ggplot(df_plot, aes(x = delta_qaly, y = delta_cost)) +
  ...
  labs(
    title = "ΔCost vs ΔQALY - Mean delle simulazioni",
